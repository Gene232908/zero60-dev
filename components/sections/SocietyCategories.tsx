...
export function SocietyCategories() {
  return (
    <section className="shell py-[var(--section-y)]">
      ...
      <Reveal stagger="loose" variant="fade" weight="tertiary">
        {SOCIETY_CATEGORIES.map((category) => (
          <article key={category.index} className="group relative border-t border-line py-9 md:py-12">
            <div className="grid grid-cols-12 items-baseline gap-x-6 gap-y-3">
              <p className="eyebrow col-span-12 sm:col-span-1">{category.index}</p>

              <h3 className="display col-span-12 text-[clamp(1.6rem,3.6vw,2.6rem)] sm:col-span-5">
                {category.title}
              </h3>

              <p className="col-span-12 text-sm leading-relaxed text-fg-muted sm:col-span-6">
                {category.description}
              </p>
            </div>

            {/* Hairline draws in from the left on hover — the elegant equivalent
                of the Productions row shift. */}
            <span
              aria-hidden="true"
              className="absolute bottom-0 left-0 h-px w-full origin-left scale-x-0 bg-accent transition-transform duration-[var(--dur-slow)] ease-[var(--ease-out)] group-hover:scale-x-100"
            />
          </article>
        ))}
      </Reveal>
